...
namespace ShipConnect.CQRS.ChatCQRS.Commands
{
    ...
    public class SendChatMessageCommandHandler : IRequestHandler<SendChatMessageCommand, GeneralResponse<SendChatMessageDTO>>
    {
        ...
        {
            if (request.DTO.SenderId == request.DTO.ReceiverId)
            {
                return GeneralResponse<SendChatMessageDTO>
                    .FailResponse("SenderId and ReceiverId must be different.");
            }
            try
            {
